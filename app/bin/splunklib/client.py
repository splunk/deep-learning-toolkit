...
class Endpoint(object):
    ...
    def put(self, path_segment="", owner=None, app=None, sharing=None, **query):
        if path_segment.startswith('/'):
            path = path_segment
        else:
            path = self.service._abspath(self.path + path_segment, owner=owner, app=app, sharing=sharing)
        return self.service.put(path, owner=owner, app=app, sharing=sharing, **query)
...
class Entity(Endpoint):
    ...
    def put(self, path_segment="", owner=None, app=None, sharing=None, **query):
        owner, app, sharing = self._proper_namespace(owner, app, sharing)
        return super(Entity, self).put(path_segment, owner=owner, app=app, sharing=sharing, **query)
